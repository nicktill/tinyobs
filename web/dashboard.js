...
let chartsPerServiceRaw = parseInt(
  localStorage.getItem("tinyobs-charts-per-service") || "6",
);
let chartsPerService = Number.isNaN(chartsPerServiceRaw)
  ? 6
  : chartsPerServiceRaw === 999
    ? 999
    : Math.min(chartsPerServiceRaw, 12);
...
function exportConfig() {
  ...
  a.href = url;
  a.download =
    "tinyobs-config-" + new Date().toISOString().split("T")[0] + ".json";
...
function importConfig(event) {
  ...
  reader.onload = function (e) {
    try {
      ...
      if (config.timeRange) {
        ...
        document.querySelectorAll(".time-btn").forEach((btn) => {
          btn.classList.toggle(
            "active",
            btn.getAttribute("data-range") === config.timeRange,
          );
        });
      }
      ...
      // Restore filters
      if (config.filters) {
        if (config.filters.service)
          document.getElementById("serviceFilter").value =
            config.filters.service;
        if (config.filters.endpoint)
          document.getElementById("endpointFilter").value =
            config.filters.endpoint;
        if (config.filters.metricName)
          document.getElementById("metricNameFilter").value =
            config.filters.metricName;
      }
  ...
  try {
    ...
    if (statsRes.ok) {
      ...
      document.getElementById("totalMetrics").textContent =
        stats.TotalMetrics?.toLocaleString() || "0";
      document.getElementById("totalSeries").textContent =
        stats.TotalSeries?.toLocaleString() || "0";
    }

    if (storageRes.ok) {
      const storage = await storageRes.json();
      const usedPct = ((storage.used_bytes / storage.max_bytes) * 100).toFixed(
        1,
      );
      document.getElementById("storageSize").textContent =
        formatBytes(storage.used_bytes) +
        " / " +
        formatBytes(storage.max_bytes) +
        " (" +
        usedPct +
        "%)";
    }
...
async function loadDashboard() {
  ...
  try {
    ...
    updateDashboardFilters(
      ...
      Array.from(endpoints).sort(),
      Array.from(metricNames).sort(),
    );
...
function updateDashboardFilters(services, endpoints, metricNames) {
  ...
  serviceFilter.innerHTML =
    '<option value="all">All Services</option>' + serviceOptions;
  exploreServiceFilter.innerHTML =
    '<option value="all">All Services</option>' + serviceOptions;
  ...
  endpointFilter.innerHTML =
    '<option value="all">All Endpoints</option>' + endpointOptions;
  ...
  const metricOptions = metricNames
    ...
    .join("");
  metricNameFilter.innerHTML =
    '<option value="all">All Metrics</option>' + metricOptions;
}
...
function renderDashboard(services) {
  ...
  const hideSystemMetrics =
    document.getElementById("hideSystemMetrics")?.checked ?? true;
  ...
  if (servicesToShow.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📊</div><p>No metrics for this service</p></div>';
    return;
  }

  container.innerHTML = servicesToShow
    .map(([service, metrics]) => {
      // Apply filters
      ...
      if (selectedEndpoint !== "all") {
        filteredMetrics = filteredMetrics.filter(
          (m) =>
            m.labels?.endpoint === selectedEndpoint ||
            m.labels?.path === selectedEndpoint,
        );
      }
      ...
      const chartCards = metricsToShow
        .map(([name, data], idx) => {
  ...
  setTimeout(() => {
    servicesToShow.forEach(([service, metrics]) => {
      ...
      // Respect hide system metrics when rendering (was only applied to cards before)
      if (hideSystemMetrics) {
        filteredMetrics = filteredMetrics.filter(
          (m) =>
            !m.name.startsWith("go_") &&
...
async function renderServiceChart(
  ...
  try {
    ...
    const response = await fetch(
      `/v1/query/range?metric=${encodeURIComponent(metricName)}&start=${start}&end=${end}&maxPoints=200`,
    );
    ...
    if (selectedEndpoint !== "all") {
      filteredSeries = filteredSeries.filter(
        (series) =>
          series.labels?.endpoint === selectedEndpoint ||
          series.labels?.path === selectedEndpoint,
      );
    }
    ...
    if (comparisonMode) {
      const compareStart = new Date(
        now - (hours + 24) * 60 * 60 * 1000,
      ).toISOString();
      const compareEnd = new Date(now - 24 * 60 * 60 * 1000).toISOString();

      const compareResponse = await fetch(
        `/v1/query/range?metric=${encodeURIComponent(metricName)}&start=${compareStart}&end=${compareEnd}&maxPoints=200`,
      );
      if (compareResponse.ok) {
        ...
        if (compareResult.data && compareResult.data.length > 0) {
          ...
          if (selectedEndpoint !== "all") {
            compareFilteredSeries = compareFilteredSeries.filter(
              (series) =>
                series.labels?.endpoint === selectedEndpoint ||
                series.labels?.path === selectedEndpoint,
            );
...
function setChartsPerService() {
  ...
  chartsPerService = value === "999" ? 999 : parseInt(value);
  if (Number.isNaN(chartsPerService)) chartsPerService = 6;
  localStorage.setItem(
    "tinyobs-charts-per-service",
    chartsPerService.toString(),
  );
...
function getMetricStatus(metricName, avgValue, latestValue, trend) {
  ...
  if (!threshold) {
    ...
    if (trend < -0.2)
      return { icon: "↓", color: "#56d364", label: "Declining" };
    return { icon: "→", color: "#8b949e", label: "Stable" };
  }
...
function calculateTrend(points) {
  ...
  const firstAvg =
    firstHalf.reduce((sum, p) => sum + p.y, 0) / firstHalf.length;
  const secondAvg =
    secondHalf.reduce((sum, p) => sum + p.y, 0) / secondHalf.length;
...
async function openChartModal(metricName, service) {
  try {
    ...
    const response = await fetch(
      `/v1/query/range?metric=${encodeURIComponent(metricName)}&start=${start}&end=${end}&maxPoints=500`,
    );
...
function renderMetricsBrowser() {
  ...
  if (!metricsData || metricsData.length === 0) {
    browser.innerHTML =
      '<div class="empty-state"><div class="empty-icon">📊</div><p>No metrics available</p></div>';
    return;
  }
  ...
  const filtered = Object.entries(grouped).filter(([key, data]) => {
    ...
    const matchesType =
      typeFilter === "all" || inferMetricType(data.name) === typeFilter;
  ...
  if (filtered.length === 0) {
    browser.innerHTML =
      '<div class="empty-state"><p>No metrics match your filters</p></div>';
    return;
  }

  browser.innerHTML = filtered
    .map(([key, data]) => {
      ...
      const labelsHtml =
        Object.keys(data.labels).length > 0
          ? Object.entries(data.labels)
              ...
              .map(
                ([k, v]) =>
                  `<div class="label"><span class="label-key">${k}</span>=<span class="label-value">"${v}"</span></div>`,
              )
              .join("")
...
function inferMetricType(name) {
  ...
  if (name.includes("_bucket") || name.includes("_duration"))
    return "histogram";
  return "gauge";
}
...
async function renderExploreChart() {
  ...
  try {
    const promises = Array.from(metricNames).map(async (name) => {
      const response = await fetch(
        `/v1/query/range?metric=${encodeURIComponent(name)}&start=${start}&end=${end}&maxPoints=500`,
      );
    ...
    results.forEach((result) => {
      ...
      result.data.forEach((series) => {
        ...
        const label =
          series.labels && Object.keys(series.labels).length > 0
            ? `${series.metric}{${Object.entries(series.labels)
                .filter(([k]) => !k.startsWith("__"))
                .map(([k, v]) => `${k}="${v}"`)
                .join(", ")}}`
            : series.metric;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!["6", "12", "999"].includes(chartsPerService.toString())) {
    chartsPerService = 6;
  }
